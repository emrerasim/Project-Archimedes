/****************************************************************************************
   ...
   Bu dosya Project-Archimedes için Türkçe açıklamalarla uyarlanmıştır.
   sTune, açık çevrim PID autotune işlemini S-eğrisi yöntemiyle gerçekleştirir.
 ****************************************************************************************/
...
// Kayar pencere ortalaması için global tanjant yardımcı sınıfı.
sTan::sTan() {}
sTan tangent = sTan();
...
void sTune::Reset() {
    // Tuning durumunu ve sayaçları sıfırla.
    _tunerStatus = test;
    if (_output != nullptr) {
...
void sTune::Configure(const float inputSpan, const float outputSpan, float outputStart, float outputStep,
    ...
    // Parametreleri yükle ve test yapılandırmasını hazırla.
    sTune::Reset();
    ...
    _samples = samples;
    _bufferSize = (uint16_t)(_samples * 0.06f);
...
uint8_t sTune::Run() {
    ...
    switch (_tunerStatus) {
        ...
        case test: // eğim noktası (inflection point) test yöntemi
            ...
            if (settleElapsed >= _settlePeriodUs) { // yerleşme zamanı bitti
                if (sampleCount == 1 && _output != nullptr) {
                    *_output = _outputStep;
                }
                if (usElapsed >= _samplePeriodUs) { // örnekleme zamanı
                    usPrev = usNow;
                    if (sampleCount <= _samples) { // test devam ediyor

                        // pvInst ve pvAvg çözünürlükleri
                        ...
                        if (sampleCount == 0) { // ilk örnekleme
                            tangent.init(pvInst);
                            pvAvg = pvInst;
                        ...
                        // Eğrinin eğimini ölçmek için kayan tanjant kullanılır.
                        // https://en.wikipedia.org/wiki/Inflection_point
                        pvTangent = pvAvg - tangent.startVal();

                        // dead time (ölü zaman) tespiti
                        bool dt = false;
                        ...
                        // inflection point (eğrilik kırılması) tespiti
                        bool ipcount = false;
                        if (_action == directIP || _action == direct5T) {
                            if (pvTangent > slopeIp + epsilon) ipcount = true;
                            if (pvTangent < 0 + epsilon) ipCount = 0; // düz ya da negatif eğim
                        } else { // reverse
                            if (pvTangent < slopeIp - epsilon) ipcount = true;
                            if (pvTangent > 0 - epsilon) ipCount = 0; // düz ya da pozitif eğim
                        }
                        if (ipcount) {
                            ipCount = 0;
                            slopeIp = pvTangent;
                        ...
                        if ((_action == directIP || _action == reverseIP) && (ipCount == ((uint16_t)(_samples / 16)))) {
                            ...
                            // pvStart ile pvMax arasındaki tangent üzerinden Tu hesapla
                            _Tu = (((pvMax - pvStart) / slopeIp) * _tangentPeriodUs * 0.000001f) - _td;
                        }

                        if (_action == direct5T || _action == reverse5T) {
                            // 5Tau testi: tepe değer yakalanana kadar devam
                            if (sampleCount >= _samples - 1) sampleCount = _samples - 2;
                            if (us > _testTimeSec * 100000) { // test süresinin %10'u
                                if (pvAvg > pvPk) {
                                ...
                                if (pvPkCount == ((uint16_t)(1.2f * _bufferSize))) {
                                    // test tamamlandı
                                    pvPkCount++;
                                    sampleCount = _samples;
                                    pvMax = pvAvg + (pvInst - pvStart) * 0.05f;
                                    _Tu = (us * 1.6667f * 0.000001f * 0.286f) - _td;
                                }
                            }
                        }

                        if (sampleCount == _samples) { // test bitti
                            _R = _td / _Tu;
...
void sTune::printResults() {
    if (_serialMode == printALL || _serialMode == printDEBUG || _serialMode == printSUMMARY) {
        ...
        // Kontrol edilebilirlik göstergesi
        ...
        if (controllability > 0.75f) Serial.println(F(" (easy to control)"));
        else if (controllability > 0.25f) Serial.println(F(" (average controllability)"));
        else Serial.println(F(" (difficult to control)"));

        // Örnekleme periyodu kontrolü
...
// PID değerlerini dışarıya verir.
...
float sTune::softPwm(const uint8_t relayPin, float input, float output, float setpoint, uint32_t windowSize, uint8_t debounce) {
    ...
    // SSR için optimum tetikleme düzenlemesi
